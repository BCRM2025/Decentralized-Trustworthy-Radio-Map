...
npz_file2 = np.load('npz/Building-A-0.5.npz')
test1 = npz_file2['arr_0']
train1 = npz_file2['arr_1']
q = test1[:, :2]


class Environment(object):

    def __init__(self,  attack_param: dict = None, sensor_param: dict = None,
                 consensus_param:dict = None, network_param:dict = None, 
                 genesis_blockheadextra:dict = None, genesis_blockextra:dict = None,
                 dataitem_param:dict = None, K: int = 5, T: int = 3, b: float = 2.5):
        """initiate the running environment

        Param
        -----
        attack_param: attack parameters (dict)
        sensor_param: sensor parameters (dict)
        ...
        K, T, B: space-time anomaly detection parameters

        """

        self.key = 1
        self.n = 50  # map generation frequency
        self.m = 50  # sensor upload frequency
        self.malicious_num = 0
        self.r = 0 # time decay factor
        self.b_num = None
        self.num_rounds = None

        #environment parameters
        self.miner_num = global_var.get_miner_num()
        self.sensor_num = global_var.get_sensor_num()  # number of sensors
        self.total_round = 0
        self.sensor_data1 = train1
        ...
        for miner_id in range(self.miner_num):
            miner = Miner(miner_id, consensus_param, dataitem_param['max_block_capacity'], dataitem_param['dataitem_input_interval'] == 0, K=K, T=T, b=b)
        ...
        for sensor_id in range(self.sensor_num):
            S = Sensor(sensor_id, sensor_param, 0)
            if global_var.get_sig():
                S.read_key_pairs('SK', 'PK')
            self.sensors.append(S)
            if S.type == 1:
                self.malicious_num += 1
                CHAIN_DATA_PATH = global_var.get_chain_data_path()
                with open(CHAIN_DATA_PATH / f'Malicious_nodes.txt', 'a') as f:
                    print(f"ID {S.Sensor_ID}\n", file=f)
    ...
    def exec(self, num_rounds, max_height, process_bar_type, post_verification):
        ...
        for temp_miner in self.miners:
            temp_miner.set_detection_enabled(global_var.get_detect())
        try:
            for round in range(1, num_rounds+1):
                if round >= 500: # 如果存在不诚实节点，500轮后开始攻击
                    self.key = 1
                if round == 1:
                    for temp_sensor in self.sensors:
                        temp_sensor.get_data(copy.deepcopy(self.sensor_data1[temp_sensor.Sensor_ID, :]))
                ...
                if round == 1:
                    for temp_sensor in self.sensors:
                        temp_sensor.probability = 0.5
                        temp_sensor.data_process(round, self.key)
                    for temp_miner in self.miners:
                        temp_miner.query_sensor(self.sensors, self.query_positions, round)
                        if flag == 1 and global_var.get_fastrun():
                            Map = temp_miner.data_process(self.r)
                            inputfromz = hashG(Map)
                            update = 1
                            flag = 0
                        if global_var.get_fastrun():
                            temp_miner.get_map(Map)
                        else:
                            Map = temp_miner.data_process(0)
                            inputfromz = hashG(Map)
                        temp_miner.input_tape.append(("INSERT", inputfromz))
                    for temp_sensor in self.sensors:
                        temp_sensor.probability = 0.25
                ...
                for miner in self.miners:
                    ...
                    # 处理诚实矿工
                    if round % self.m == 0 and flag == 1:
                        miner.query_sensor(self.sensors, self.query_positions, round)

                    if round % self.n == 0:
                        if flag == 1 and global_var.get_fastrun():
                            Map = miner.data_process(self.r)
                            inputfromz = hashG(Map)
                            update = 1
                            flag = 0
                        if global_var.get_fastrun():
                    ...
                    if (new_msgs := miner.BackboneProtocol(round, self.state)) is not None:
                        # self.network.access_network(new_msgs,temp_miner.miner_id,round)
                        for msg in new_msgs:
                            if isinstance(msg, Block):
                                if global_var.get_showmap() and update == 1:
                                    update = 0
                                    if msg.map.any():
                                        self.b_num = msg.name
                                        self.plot_strongest_path_AoD_map(msg.map, round)
    ...
    def evaluation(self, Map, state):
        ...
        # CHAIN_DATA_PATH = global_var.get_chain_data_path()
        # with open(CHAIN_DATA_PATH / 'sensordata.txt', 'a') as f:
        #     for row in self.sensor_data1:
        #         print(f"{row}\n", file=f)
        # with open(CHAIN_DATA_PATH / 'tx1data.txt', 'a') as f:
        #     for row in tx1:
        #         print(f"{row}\n", file=f)

        r = root_mean_squared_error(true_values, pred_values)
        return r

    def plot_strongest_path_AoD_map(self, x, y):
        # if y < self.num_rounds / 3 or y >= 2 * self.num_rounds / 3:
        #     b = 13
        # else:
        #     b = 9
        ...
        # 获取第三列的最大值和最小值
        max_value = np.max(col3)
        min_value = np.min(col3)
        xgrid = np.linspace(np.min(x[:, 0]), np.max(x[:, 0]), 3000)
        ygrid = np.linspace(np.min(x[:, 1]), np.max(x[:, 1]), 3000)
        xgrid, ygrid = np.meshgrid(xgrid, ygrid)
        zgrid = griddata(self.query_positions, pred_phi, (xgrid, ygrid))
        v = np.array([a] + list(np.linspace(min_value, max_value, 13, endpoint=True)))
        tick = v.flatten()
        ...
        # plt.title(f'Received Power Map of Block {self.b_num} at round {y}')
        # cbar.set_label('Received Power(dBm)', labelpad=21, rotation=270, fontsize=14)
        PIC_PATH = global_var.get_pic_result_path()
        fig_name = PIC_PATH / f'map of block {self.b_num}.jpg'
        # fig_name1 = PIC_PATH / f'map of block {self.b_num}.eps'
        # fig_name2 = PIC_PATH / f'map of block {self.b_num}.svg'
    ...
    def view_and_write(self, quantile):
        ...
        MSE_PATH = global_var.get_mse_result_path()
        ...
        if global_var.get_evaluation():
            # 打印链信息
            q: list[Block] = [self.global_chain.head]
            blocklist = []
            while q:
                block = q.pop(0)
                ...
                for i in block.next:
                    q.append(i)
            # K = blocklist[0].sensor_data.shape[0]
            H = blocklist[0].height
            for block in blocklist:
                # if block.sensor_data.shape[0] > K and block.sensor_data.shape[0] > 0:
                #     K = block.sensor_data.shape[0]
                if block.height > H and block.sensor_data.any():
                    # npzfile1 = np.load('test.npz')
                    # map = npzfile1['arr_1']
                    H = block.height
                    mse = self.evaluation(block.map, block.state)
                    rmse1 = np.append(rmse1, [mse], axis=0)

                    with open(MSE_PATH / f'rmse_data_{str(block.name)}.txt', 'a') as f:
                        print("Map maintained by block", block.name, file=f)
                        print(f"RMSE of radio map: {mse}", file=f)
                        print(f"data_num: {block.sensor_data.shape[0]}\n", file=f)
