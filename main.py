...
def main(**args):
    ...
    sensor_num = args.get('sensor_num') or int(env_config['sensor_num'])
    K_num = args.get('K')
    T_num = args.get('T')
    B_num = args.get('B')
    ...
    global_var.set_sensor_num(sensor_num)
    global_var.set_K(K_num)
    global_var.set_T(T_num)
    global_var.set_B(B_num)
    global_var.set_data(args.get('data'))
    ...
    # 设置Map配置
    global_var.set_evaluation(args.get('evaluation') or config.getboolean('MapSettings', 'evaluation'))
    global_var.set_fastrun(args.get('fast_run') or config.getboolean('MapSettings', 'fast_run'))
    global_var.set_showmap(args.get('show_map') or config.getboolean('MapSettings', 'show_map'))
    global_var.set_detect(args.get('detect') or config.getboolean('MapSettings', 'detect'))
    global_var.set_sig(args.get('sig') or config.getboolean('MapSettings', 'sig'))
    ...
    # 设置sensor参数
    sensor_setting = dict(config['SensorSettings'])
    sensor_param = {
        'RT_TX_POWER_dBm': args.get('RT_TX_POWER_dBm') if args.get('RT_TX_POWER_dBm') is not None else
        int(sensor_setting['RT_TX_POWER_dBm']),
        'Pt_dBm': args.get('Pt_dBm') if args.get('Pt_dBm') is not None else
        int(sensor_setting['Pt_dBm']),
        'noise_power_density_dBm_Hz': args.get('noise_power_density_dBm_Hz') if args.get(
            'noise_power_density_dBm_Hz') is not None else
        int(sensor_setting['noise_power_density_dBm_Hz']),
        'NUM_PATHS_TO_LEARN': args.get('NUM_PATHS_TO_LEARN') if args.get('NUM_PATHS_TO_LEARN') is not None else
        int(sensor_setting['NUM_PATHS_TO_LEARN']),
        'INFORMATION_PER_PATH': args.get('INFORMATION_PER_PATH') if args.get('INFORMATION_PER_PATH') is not None else
        int(sensor_setting['INFORMATION_PER_PATH']),
        'POWER_SENSITIVITY': args.get('POWER_SENSITIVITY') if args.get('POWER_SENSITIVITY') is not None else
        int(sensor_setting['POWER_SENSITIVITY']),
    }
    ...
    Z = Environment(attack_param, sensor_param, consensus_param,
                    network_param, genesis_blockheadextra, genesis_blockextra,
                    dataitem_param, K=args.get('K'), T=args.get('T'), b=args.get('B'))
...
could be performed with attackers designed in the simulator'
    ...
    env_setting.add_argument('--sensor_num', help='The total sensor number in the network.', type=int)
    env_setting.add_argument('--K', type=int, default=5, help='Number of nearest neighbors')
    env_setting.add_argument('--T', type=int, default=3, help='Number of historical rounds')
    env_setting.add_argument('--B', type=float, default=2.5, help='Standard deviation multiplier')
    ...
    # MapSettings
    map_setting = parser.add_argument_group('MapSettings', 'Settings for Radio Map')
    map_setting.add_argument('--evaluation', help='Output mse of every radio map in the blockchain.',
                             action='store_true')
    map_setting.add_argument('--fast_run', help='One data prediction each updated round.',
                             action='store_true')
    map_setting.add_argument('--show_map', help='Draw the radio map.', action='store_true')
    map_setting.add_argument('--sig', help='Start digital signature using RSA.', action='store_true')
    map_setting.add_argument('--detect', help='Detect dishonest behavior.', action='store_true')
