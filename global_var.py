...
def __init__(result_path:Path = None): 
    ...
    PIC_RESULT_PATH = RESULT_PATH / 'Pic'
    PIC_RESULT_PATH.mkdir(parents=True,exist_ok = True)
    MSE_RESULT_PATH = RESULT_PATH / 'MSE'
    MSE_RESULT_PATH.mkdir(parents=True,exist_ok = True)
    ...
    _var_dict['PIC_RESULT_PATH'] = PIC_RESULT_PATH
    _var_dict['MSE_RESULT_PATH'] = MSE_RESULT_PATH
    ...
    _var_dict['EVALUATION'] = False
    _var_dict['FASTRUN'] = False
    _var_dict['SHOWMAP'] = False
    _var_dict['SIG'] = False
    _var_dict['DETECT'] = False
    _var_dict['DATA'] = 0

def set_detect(detect):
    """设置是否启用space-time anomaly detection"""
    _var_dict['DETECT'] = detect
def get_detect():
    """获得是否启用space-time anomaly detection"""
    return _var_dict['DETECT']

def set_evaluation(evaluation):
    """设置是否计算误差"""
    _var_dict['EVALUATION'] = evaluation
def get_evaluation():
    """获得是否计算误差"""
    return _var_dict['EVALUATION']

def set_fastrun(fastrun):
    """设置是否快速运行"""
    _var_dict['FASTRUN'] = fastrun
def get_fastrun():
    """获得是否快速运行"""
    return _var_dict['FASTRUN']

def set_showmap(showmap):
    """设置是否绘制地图"""
    _var_dict['SHOWMAP'] = showmap
def get_showmap():
    """获得是否绘制地图"""
    return _var_dict['SHOWMAP']
...
def set_sensor_num(sensor_num):
    """定义传感节点数量 type:int"""
    _var_dict['SENSOR_NUM'] = sensor_num
def get_sensor_num():
    """获得传感节点数量"""
    return _var_dict['SENSOR_NUM']
...
def get_pic_result_path():
    return _var_dict['PIC_RESULT_PATH']

def get_mse_result_path():
    return _var_dict['MSE_RESULT_PATH']
