...
class PoW(Consensus):
    ...
    def mining_consensus(self, miner_id:bytes, isadversary, x, sent_data, Map, state, round):
        '''计算PoW\n
        param:
        ...
        # 将 x 转换为 bytes
        if isinstance(x, str):
            x = bytes.fromhex(x)  # 假设 x 是十六进制字符串
        ...
        while i < self.q:
            ...
            if currenthash<self.target:
                ...
                blocknew = self.Block(blockhead,b_last, state, round,
                                      isadversary,global_var.get_blocksize(), sent_data, Map)
                self.ctr = 0
