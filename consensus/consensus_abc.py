...
class Consensus(metaclass=ABCMeta):        #抽象类
    ...
    class Block(data.Block):
        ...
        def __init__(self, blockhead: data.BlockHead, preblock: data.Block = None,
                     state = None, round = None, isadversary = False, blocksize_MB = 2,
                     sent_data=np.array([]).reshape(-1, 5), Map=None):
            ...
            super().__init__(f"B{block_number}", blockhead, height,
                             state, round, isadversary, is_genesis,
                             blocksize_MB, sent_data, Map)
    ...
    def consensus_process(self, isadversary, x, sent_data, Map, state, round):
        '''典型共识过程：挖出新区块并添加到本地链
        return:
            msg_list 包含挖出的新区块的列表，无新区块则为None type:list[Block]/None
            msg_available 如果有新的消息产生则为True type:Bool
        '''
        newblock, success = self.mining_consensus(self.miner_id_bytes, isadversary, x,
                                                  sent_data, Map, state, round)
        if success is False:
            return None, False
    ...
    @abstractmethod
    def mining_consensus(self, miner_id, isadversary, x, sent_data, Map, state, round):
        '''共识机制定义的挖矿算法
        return:
            新产生的区块  type:Block 
            挖矿成功标识    type:bool
